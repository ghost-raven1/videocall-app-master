...
const baseUrl = process.env.E2E_BASE_URL || "http://192.168.0.105:3000";
const password = process.env.E2E_PASSWORD || "dev_admin_password";
const executablePath = process.env.E2E_CHROMIUM_PATH || "/usr/bin/chromium-browser";
...
async function login(page, label) {
  ...
  if (hasPasswordInput) {
    ...
    // Some environments auto-authenticate and redirect from /login to /
...
async function run() {
  ...
  try {
    ...
    await page1.waitForURL(/\/(join|call)\//, { timeout: 25000 });

    let roomId = null;
    const createFlowUrl = page1.url();
    if (createFlowUrl.includes("/call/")) {
      roomId = createFlowUrl.split("/call/")[1]?.split(/[?#]/)[0] || null;
      console.log("Create flow: direct call route");
    } else if (createFlowUrl.includes("/join/")) {
      const shortCode = createFlowUrl.split("/join/")[1]?.split(/[?#]/)[0];
      if (!shortCode) {
        throw new Error("Failed to extract room short code");
      }
      console.log("Create flow: join route, short code:", shortCode);
      await page1.getByRole("button", { name: /Присоединиться к звонку|Join call/i }).click();
      await page1.waitForURL(/\/call\//, { timeout: 25000 });
      roomId = page1.url().split("/call/")[1]?.split(/[?#]/)[0] || null;
    }

    if (!roomId) {
      throw new Error("Failed to extract room ID after create flow");
    }
    console.log("[P1] call URL:", page1.url());

    await page2.goto(`${baseUrl}/call/${roomId}`, { waitUntil: "domcontentloaded" });
